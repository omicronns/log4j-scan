...
- Supports DNS callback for vulnerability discovery and validation, via dns log file.
- WAF Bypass payloads.
...
usage: log4j-scan.py [-h] [-u URL] [-p PROXY] [-l USEDLIST] [--request-type REQUEST_TYPE] [--headers-file HEADERS_FILE] [--run-all-tests] [--exclude-user-agent-fuzzing] [--wait-time WAIT_TIME] [--waf-bypass] [--verbose] [--callback-domain CALLBACK_DOMAIN]
                     [--dns-logs-path DNS_LOGS_PATH]
...
options:
  ...
  --callback-domain CALLBACK_DOMAIN
                        Callback domain [Default: example.com].
  --dns-logs-path DNS_LOGS_PATH
                        DNS logs path [Default: /data/dns.log].
```
